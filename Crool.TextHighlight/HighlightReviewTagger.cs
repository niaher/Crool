namespace Crool.TextHighlight
{
    ...
    internal class HighlightReviewTagger : ITagger<HighlightReviewTag>
    {
        private object updateLock = new object();

        public HighlightReviewTagger(ITextView view, ITextBuffer sourceBuffer, ITextSearchService textSearchService, ITextStructureNavigator textStructureNavigator)
        {
            this.View = view;
        ...
        public IEnumerable<ITagSpan<HighlightReviewTag>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            {
                yield return new TagSpan<HighlightReviewTag>(currentWord, new HighlightReviewTag());
            }

            // Second, yield all the other words in the file
            foreach (SnapshotSpan span in NormalizedSnapshotSpanCollection.Overlap(spans, wordSpans))
            {
                yield return new TagSpan<HighlightReviewTag>(span, new HighlightReviewTag());
            }
        }
        ...
        {
            SnapshotPoint? point = caretPosition.Point.GetPoint(this.SourceBuffer, caretPosition.Affinity);
        ...
        private void UpdateWordAdornments()
        {
            ...
            TextExtent word = this.TextStructureNavigator.GetExtentOfWord(currentRequest);
            bool foundWord = true;
            ...
            {
                this.SynchronousUpdate(currentRequest, new NormalizedSnapshotSpanCollection(wordSpans), currentWord);
            }
        }
        ...
        private void SynchronousUpdate(SnapshotPoint currentRequest, NormalizedSnapshotSpanCollection newSpans, SnapshotSpan? newCurrentWord)
        {
            ...
            {
                if (currentRequest != this.RequestedPoint) return;
